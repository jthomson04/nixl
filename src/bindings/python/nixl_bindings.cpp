...
PYBIND11_MODULE(_bindings, m) {
    ...
    py::class_<nixlAgent>(m, "nixlAgent")
        ...
        .def("createXferReq", [](nixlAgent &agent,
                   ...
                   py::arg("backend") = std::vector<uintptr_t>({}))
        .def("estimateXferCost", [](nixlAgent &agent, uintptr_t reqh) -> std::chrono::duration<double> {
                std::chrono::duration<double> duration;
                nixl_status_t status = agent.estimateXferCost(reinterpret_cast<nixlXferReqH&>(reqh), duration);
