...
{
    if (!name || !agent) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
{
    if (!agent) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
{
    if (!agent || !data || !len) { return NIXL_CAPI_ERROR_INVALID_PARAM; }

    try {
        nixl_blob_t   blob;
        nixl_status_t ret = agent->inner->getLocalMD(blob);
        if (ret != NIXL_SUCCESS) { return NIXL_CAPI_ERROR_BACKEND; }

        // Allocate memory for the blob data
        void* blob_data = malloc(blob.size());
        if (!blob_data) { return NIXL_CAPI_ERROR_BACKEND; }
...
nixl_capi_status_t nixl_capi_load_remote_md(
        nixl_capi_agent_t agent, const void* data, size_t len, char** agent_name)
{
    if (!agent || !data || !len || !agent_name) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
    ...
    try {
        ...
        if (!name_str) { return NIXL_CAPI_ERROR_BACKEND; }
        *agent_name = name_str;
...
{
    if (!agent || !remote_agent) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
nixl_capi_status_t nixl_capi_get_available_plugins(
        nixl_capi_agent_t agent, nixl_capi_string_list_t* plugins)
{
    if (!agent || !plugins) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
{
    if (!list) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
{
    if (!list || !size) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
nixl_capi_status_t nixl_capi_string_list_get(
        nixl_capi_string_list_t list, size_t index, const char** str)
{
    if (!list || !str || index >= list->strings.size()) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
nixl_capi_status_t nixl_capi_get_plugin_params(nixl_capi_agent_t agent, const char* plugin_name,
        nixl_capi_mem_list_t* mems, nixl_capi_params_t* params)
{
    if (!agent || !plugin_name || !mems || !params) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
{
    if (!params) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
nixl_capi_status_t nixl_capi_create_backend(nixl_capi_agent_t agent, const char* plugin_name,
        nixl_capi_params_t params, nixl_capi_backend_t* backend)
{
    if (!agent || !plugin_name || !params || !backend) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
{
    if (!backend) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
{
    if (!args) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
nixl_capi_status_t nixl_capi_opt_args_add_backend(
        nixl_capi_opt_args_t args, nixl_capi_backend_t backend)
{
    if (!args || !backend) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
nixl_capi_status_t nixl_capi_opt_args_set_notif_msg(
        nixl_capi_opt_args_t args, const void* data, size_t len)
{
    if (!args || (!data && len > 0)) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
nixl_capi_status_t nixl_capi_opt_args_get_notif_msg(
        nixl_capi_opt_args_t args, void** data, size_t* len)
{
    if (!args || !data || !len) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
    ...
    try {
        ...
        if (!msg_data) { return NIXL_CAPI_ERROR_BACKEND; }
...
{
    if (!args || !has_notif) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
nixl_capi_status_t nixl_capi_opt_args_set_skip_desc_merge(
        nixl_capi_opt_args_t args, bool skip_merge)
{
    if (!args) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
nixl_capi_status_t nixl_capi_opt_args_get_skip_desc_merge(
        nixl_capi_opt_args_t args, bool* skip_merge)
{
    if (!args || !skip_merge) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
{
    if (!params || !is_empty) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
nixl_capi_status_t nixl_capi_params_create_iterator(
        nixl_capi_params_t params, nixl_capi_param_iter_t* iter)
{
    if (!params || !iter) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
{
    if (!iter || !key || !value || !has_next) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
{
    if (!iter) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
{
    if (!list || !is_empty) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
nixl_capi_status_t nixl_capi_mem_list_get(
        nixl_capi_mem_list_t list, size_t index, nixl_capi_mem_type_t* mem_type)
{
    if (!list || !mem_type || index >= list->mems.size()) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
{
    if (!str) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
nixl_capi_status_t nixl_capi_get_backend_params(nixl_capi_agent_t agent,
        nixl_capi_backend_t backend, nixl_capi_mem_list_t* mems, nixl_capi_params_t* params)
{
    if (!agent || !backend || !mems || !params) { return NIXL_CAPI_ERROR_INVALID_PARAM; }

    try {
        auto mem_list = new nixl_capi_mem_list_s;
        auto param_list = new nixl_capi_params_s;

        nixl_status_t ret = agent->inner->getBackendParams(
                backend->backend, mem_list->mems, param_list->params);
...
nixl_capi_status_t nixl_capi_create_xfer_dlist(
        nixl_capi_mem_type_t mem_type, nixl_capi_xfer_dlist_t* dlist)
{
    if (!dlist) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
{
    if (!dlist || !len) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
nixl_capi_status_t nixl_capi_xfer_dlist_has_overlaps(
        nixl_capi_xfer_dlist_t dlist, bool* has_overlaps)
{
    if (!dlist || !has_overlaps) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
nixl_capi_status_t nixl_capi_create_reg_dlist(
        nixl_capi_mem_type_t mem_type, nixl_capi_reg_dlist_t* dlist)
{
    if (!dlist) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
{
    if (!agent || !dlist) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
nixl_capi_status_t nixl_capi_create_xfer_req(nixl_capi_agent_t agent, nixl_capi_xfer_op_t operation,
        nixl_capi_xfer_dlist_t local_descs, nixl_capi_xfer_dlist_t remote_descs,
        const char* remote_agent, nixl_capi_xfer_req_t* req_hndl, nixl_capi_opt_args_t opt_args)
{
    ...
    try {
        ...
        nixl_status_t ret = agent->inner->createXferReq(static_cast<nixl_xfer_op_t>(operation),
                *local_descs->dlist, *remote_descs->dlist, std::string(remote_agent), req->req,
...
{
    if (!agent || !req_hndl) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
{
    if (!req) { return NIXL_CAPI_ERROR_INVALID_PARAM; }

    if (req->req) { return NIXL_CAPI_ERROR_INVALID_STATE; }
...
{
    if (!agent || !req || !req->req) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
{
    if (!agent || !notif_map) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
{
    if (!notif_map) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
{
    if (!map || !size) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
nixl_capi_status_t nixl_capi_notif_map_get_agent_at(
        nixl_capi_notif_map_t map, size_t index, const char** agent_name)
{
    if (!map || !agent_name) { return NIXL_CAPI_ERROR_INVALID_PARAM; }

    try {
        auto it = map->notif_map.begin();
        std::advance(it, index);
        if (it == map->notif_map.end()) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
nixl_capi_status_t nixl_capi_notif_map_get_notifs_size(
        nixl_capi_notif_map_t map, const char* agent_name, size_t* size)
{
    if (!map || !agent_name || !size) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
...
nixl_capi_status_t nixl_capi_notif_map_get_notif(nixl_capi_notif_map_t map, const char* agent_name,
        size_t index, const void** data, size_t* len)
{
    if (!map || !agent_name || !data || !len) { return NIXL_CAPI_ERROR_INVALID_PARAM; }
