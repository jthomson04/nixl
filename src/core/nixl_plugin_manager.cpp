...
std::shared_ptr<nixlPluginHandle> nixlPluginManager::loadPluginFromPath(
        const std::string &plugin_path)
{
    ...
    if (!init) {
        std::cerr << "Failed to find nixl_plugin_init in " << plugin_path << ": " << dlerror()
                  << std::endl;
        dlclose(handle);
        return nullptr;
    }
    ...
    if (plugin->api_version != NIXL_PLUGIN_API_VERSION) {
        std::cerr << "Plugin API version mismatch for " << plugin_path << ": expected "
                  << NIXL_PLUGIN_API_VERSION << ", got " << plugin->api_version << std::endl;
        dlclose(handle);
        return nullptr;
    }
...
nixlPluginManager::nixlPluginManager()
{
    ...
    if (access(plugin_file.c_str(), F_OK) == 0) { loadPluginsFromList(plugin_file); }
#endif
...
void nixlPluginManager::addPluginDirectory(const std::string &directory)
{
    ...
    if (access(directory.c_str(), F_OK | R_OK) != 0) {
        std::cerr << "Plugin directory does not exist or is not readable: " << directory
                  << std::endl;
        return;
    }
