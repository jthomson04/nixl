...
nixlAgentData::~nixlAgentData()
{
    ...
    for (auto &elm : remoteSections) delete elm.second;
    ...
    for (auto &elm : backendHandles) delete elm.second;
}
...
nixl_status_t nixlAgent::getPluginParams(
        const nixl_backend_t &type, nixl_mem_list_t &mems, nixl_b_params_t &params) const
{
    ...
    if (plugin_handle) {
        ...
        if (data->backendEngines.count(type) == 0) { plugin_manager.unloadPlugin(type); }
        return NIXL_SUCCESS;
    }
...
nixl_status_t nixlAgent::getBackendParams(
        const nixlBackendH* backend, nixl_mem_list_t &mems, nixl_b_params_t &params) const
{
...
nixl_status_t nixlAgent::createBackend(
        const nixl_backend_t &type, const nixl_b_params_t &params, nixlBackendH*&bknd_hndl)
{
...
nixl_status_t nixlAgent::registerMem(
        const nixl_reg_dlist_t &descs, const nixl_opt_args_t* extra_params)
{
    backend_list_t* backend_list;
    nixl_status_t   ret;
    ...
    if (!extra_params || extra_params->backends.size() == 0) {
        ...
        for (auto &elm : extra_params->backends) backend_list->push_back(elm->engine);
    }
...
nixl_status_t nixlAgent::deregisterMem(
        const nixl_reg_dlist_t &descs, const nixl_opt_args_t* extra_params)
{
    ...
    if (!extra_params || extra_params->backends.size() == 0) {
        ...
        for (auto &elm : extra_params->backends) backend_set.insert(elm->engine);
    }
...
nixl_status_t nixlAgent::makeConnection(
        const std::string &remote_agent, const nixl_opt_args_t* extra_params)
{
    ...
    if (!extra_params || extra_params->backends.size() == 0) {
        ...
        for (auto &elm : extra_params->backends) backend_set.insert(elm->engine->getType());
    }
...
nixl_status_t nixlAgent::prepXferDlist(const std::string &agent_name,
        const nixl_xfer_dlist_t &descs, nixlDlistH*&dlist_hndl,
...
nixl_status_t nixlAgent::makeXferReq(const nixl_xfer_op_t &operation, const nixlDlistH* local_side,
        const std::vector<int> &local_indices, const nixlDlistH* remote_side,
        const std::vector<int> &remote_indices, nixlXferReqH*&req_hndl,
        const nixl_opt_args_t* extra_params) const
{
    ...
    if ((opt_args.hasNotif) && (!backend->supportsNotif())) { return NIXL_ERR_BACKEND; }
    ...
    ret = handle->engine->prepXfer(handle->backendOp, *handle->initiatorDescs, *handle->targetDescs,
            handle->remoteAgent, handle->backendHandle, &opt_args);
    if (ret != NIXL_SUCCESS) {
        delete handle;
        return ret;
    }
...
nixl_status_t nixlAgent::createXferReq(const nixl_xfer_op_t &operation,
        const nixl_xfer_dlist_t &local_descs, const nixl_xfer_dlist_t &remote_descs,
        const std::string &remote_agent, nixlXferReqH*&req_hndl,
        const nixl_opt_args_t* extra_params) const
{
    ...
    NIXL_LOCK_GUARD(data->lock);
    if (data->remoteSections.count(remote_agent) == 0) { return NIXL_ERR_NOT_FOUND; }
    ...
    ret1 = handle->engine->prepXfer(handle->backendOp, *handle->initiatorDescs,
            *handle->targetDescs, handle->remoteAgent, handle->backendHandle, &opt_args);
...
nixl_status_t nixlAgent::postXferReq(
        nixlXferReqH* req_hndl, const nixl_opt_args_t* extra_params) const
{
    nixl_status_t     ret;
    nixl_opt_b_args_t opt_args;
    ...
    ret = req_hndl->engine->postXfer(req_hndl->backendOp, *req_hndl->initiatorDescs,
            *req_hndl->targetDescs, req_hndl->remoteAgent, req_hndl->backendHandle, &opt_args);
    req_hndl->status = ret;
    return ret;
}

nixl_status_t nixlAgent::getXferStatus(nixlXferReqH* req_hndl)
{
    NIXL_LOCK_GUARD(data->lock);
...
nixl_status_t nixlAgent::queryXferBackend(const nixlXferReqH* req_hndl, nixlBackendH*&backend) const
{
    NIXL_LOCK_GUARD(data->lock);
...
nixl_status_t nixlAgent::releaseXferReq(nixlXferReqH* req_hndl)
{
    NIXL_LOCK_GUARD(data->lock);
...
nixl_status_t nixlAgent::releasedDlistH(nixlDlistH* dlist_hndl) const
{
    NIXL_LOCK_GUARD(data->lock);
    delete dlist_hndl;
    return NIXL_SUCCESS;
...
nixl_status_t nixlAgent::getNotifs(nixl_notifs_t &notif_map, const nixl_opt_args_t* extra_params)
{
...
nixl_status_t nixlAgent::genNotif(const std::string &remote_agent, const nixl_blob_t &msg,
...
nixl_status_t nixlAgent::getLocalPartialMD(
        const nixl_reg_dlist_t &descs, nixl_blob_t &str, const nixl_opt_args_t* extra_params) const
{
    ...
    if (!extra_params || extra_params->backends.size() == 0) {
        if (descs.descCount() != 0) {
            ...
            for (const auto &elm : data->backendEngines) backend_list->push_back(elm.second);
        }
    } else {
        backend_list = &tmp_list;
        for (const auto &elm : extra_params->backends) backend_list->push_back(elm->engine);
    }
...
nixl_status_t nixlAgent::loadRemoteMD(const nixl_blob_t &remote_metadata, std::string &agent_name)
{
    int                count = 0;
    nixlSerDes         sd;
    size_t             conn_cnt;
...
nixl_status_t nixlAgent::sendLocalMD(const nixl_opt_args_t* extra_params) const
{
    nixl_blob_t   myMD;
    ...
    if (extra_params && !extra_params->ipAddr.empty()) {
        data->enqueueCommWork(
                std::make_tuple(SOCK_SEND, extra_params->ipAddr, extra_params->port, myMD));
        return NIXL_SUCCESS;
    }
...
nixl_status_t nixlAgent::sendLocalPartialMD(
        const nixl_reg_dlist_t &descs, const nixl_opt_args_t* extra_params) const
{
    nixl_blob_t   myMD;
    ...
    if (data->useEtcd) {
        data->enqueueCommWork(
                std::make_tuple(ETCD_SEND, data->name + "/partial_metadata", 0, myMD));
        return NIXL_SUCCESS;
    }
    return NIXL_ERR_INVALID_PARAM;
#else
...
nixl_status_t nixlAgent::fetchRemoteMD(
        const std::string remote_name, const nixl_opt_args_t* extra_params)
{
    // If IP is provided, use socket-based communication
    if (extra_params && !extra_params->ipAddr.empty()) {
        data->enqueueCommWork(
...
nixl_status_t nixlAgent::invalidateLocalMD(const nixl_opt_args_t* extra_params) const
{
    // If IP is provided, use socket-based communication
    if (extra_params && !extra_params->ipAddr.empty()) {
        data->enqueueCommWork(
...
nixl_status_t nixlAgent::checkRemoteMD(
        const std::string remote_name, const nixl_xfer_dlist_t &descs) const
{
    NIXL_LOCK_GUARD(data->lock);
