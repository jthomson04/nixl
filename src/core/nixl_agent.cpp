...
{
    if (name.size() == 0) throw std::invalid_argument("Agent needs a name");
    ...
    for (size_t i = 0; i < backend_list->size(); ++i) {
        ...
        if (ret == NIXL_SUCCESS) {
            if (backend->supportsLocal()) {
                ...
                ret = data->remoteSections[data->name]->loadLocalData(meta_descs, backend);
                if (ret == NIXL_SUCCESS)
                    count++;
                else
        ...
        const nixl_reg_dlist_t &descs, const nixl_opt_args_t* extra_params)
{
    ...
    for (auto &backend : backend_set) {
        nixl_meta_dlist_t resp(descs.getType(), descs.isSorted());
        ...
        const std::string &remote_agent, const nixl_opt_args_t* extra_params)
{
    ...
    if (!extra_params || extra_params->backends.size() == 0) {
        ...
        if (backend_set->empty()) return NIXL_ERR_NOT_FOUND;
    } else {
...
nixl_status_t nixlAgent::loadRemoteMD(const nixl_blob_t &remote_metadata, std::string &agent_name)
{
    ...
    if (conn_cnt < 1) return NIXL_ERR_INVALID_PARAM;
    ...
    if (count == 0) return NIXL_ERR_BACKEND;
