...
public:
    ...
    virtual ~nixlBackendEngine() { delete customParams; }
    ...
    virtual nixl_status_t registerMem(
            const nixlBlobDesc &mem, const nixl_mem_t &nixl_mem, nixlBackendMD*&out) = 0;
    ...
    virtual nixl_status_t prepXfer(const nixl_xfer_op_t &operation, const nixl_meta_dlist_t &local,
            const nixl_meta_dlist_t &remote, const std::string &remote_agent,
            nixlBackendReqH*&handle, const nixl_opt_b_args_t* opt_args = nullptr) = 0;

    // Posting a request, which completes the async handle creation and posts it
    virtual nixl_status_t postXfer(const nixl_xfer_op_t &operation, const nixl_meta_dlist_t &local,
            const nixl_meta_dlist_t &remote, const std::string &remote_agent,
            nixlBackendReqH*&handle, const nixl_opt_b_args_t* opt_args = nullptr) = 0;
    ...
    virtual nixl_status_t getPublicData(const nixlBackendMD* meta, std::string &str) const
    {
    ...
    virtual nixl_status_t loadRemoteConnInfo(
    ...
    virtual nixl_status_t loadRemoteMD(const nixlBlobDesc &input, const nixl_mem_t &nixl_mem,
    ...
    virtual nixl_status_t loadLocalMD(nixlBackendMD* input, nixlBackendMD*&output)
    {
        return NIXL_ERR_BACKEND;
    }
