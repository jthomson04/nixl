...
public:
    ...
    ~nixlUcxMoRequestH()
    {
        for (auto &row : dlMatrix) {
            for (auto &p : row) {
                if (p.ldescs) { delete p.ldescs; }
                if (p.rdescs) { delete p.rdescs; }
            }
        }
    }
...
int nixlUcxMoEngine::setEngCnt(uint32_t num_host)
{
...
uint32_t nixlUcxMoEngine::getEngCnt()
{
    return _engineCnt;
}

int32_t nixlUcxMoEngine::getEngIdx(nixl_mem_t type, uint64_t devId)
{
    switch (type) {
        case VRAM_SEG:
            assert(devId < _gpuCnt);
            if (!(devId < _gpuCnt)) { return -1; }
        case DRAM_SEG:
            break;
        default:
            return -1;
...
string nixlUcxMoEngine::getEngName(const string &baseName, uint32_t eidx)
{
    return baseName + ":" + to_string(eidx);
}

string nixlUcxMoEngine::getEngBase(const string &engName)
{
...
nixl_mem_list_t nixlUcxMoEngine::getSupportedMems() const
{
...
nixl_status_t nixlUcxMoEngine::getConnInfo(std::string &str) const
{
    nixlSerDes    sd;
    ...
    for (auto &e : engines) {
        ...
        if (NIXL_SUCCESS != status) { return status; }
        sd.addStr("Value", s);
    }
...
nixl_status_t nixlUcxMoEngine::loadRemoteConnInfo(
        const string &remote_agent, const string &remote_conn_info)
{
    nixlSerDes          sd;
    ...
    if (it != remoteConnMap.end()) { return NIXL_ERR_INVALID_PARAM; }
    ...
    if (ret != sizeof(sz)) { return NIXL_ERR_MISMATCH; }
...
nixl_status_t nixlUcxMoEngine::connect(const string &remote_agent)
{
    remote_comm_it_t it = remoteConnMap.find(remote_agent);
    nixl_status_t    status;

    if (it == remoteConnMap.end()) { return NIXL_ERR_NOT_FOUND; }
...
nixl_status_t nixlUcxMoEngine::disconnect(const string &remote_agent)
{
    nixl_status_t    status;
    remote_comm_it_t it = remoteConnMap.find(remote_agent);
...
nixl_status_t nixlUcxMoEngine::registerMem(
        const nixlBlobDesc &mem, const nixl_mem_t &nixl_mem, nixlBackendMD*&out)
{
    ...
    if (eidx < 0) { return NIXL_ERR_INVALID_PARAM; }
...
nixl_status_t nixlUcxMoEngine::getPublicData(const nixlBackendMD* meta, std::string &str) const
{
...
nixl_status_t nixlUcxMoEngine::deregisterMem(nixlBackendMD* meta)
{
...
// To be cleaned up
nixl_status_t nixlUcxMoEngine::internalMDHelper(const nixl_blob_t &blob, const nixl_mem_t &nixl_mem,
        const std::string &agent, nixlBackendMD*&output)
{
    nixlUcxMoConnection conn;
    nixlSerDes          sd;
    ...
    if (ret != sizeof(md->eidx)) { return NIXL_ERR_MISMATCH; }
    ...
    for (auto &e : engines) {
        ...
        status = e->loadRemoteMD(input_int, nixl_mem, getEngName(agent, md->eidx), int_md);
        if (status != NIXL_SUCCESS) { return status; }
...
nixl_status_t nixlUcxMoEngine::loadLocalMD(nixlBackendMD* input, nixlBackendMD*&output)
{
...
nixl_status_t nixlUcxMoEngine::loadRemoteMD(const nixlBlobDesc &input, const nixl_mem_t &nixl_mem,
        const string &remote_agent, nixlBackendMD*&output)
{
    return internalMDHelper(input.metaInfo, nixl_mem, remote_agent, output);
}

nixl_status_t nixlUcxMoEngine::unloadMD(nixlBackendMD* input)
{
    nixl_status_t status;
...
nixl_status_t nixlUcxMoEngine::prepXfer(const nixl_xfer_op_t &operation,
        const nixl_meta_dlist_t &local, const nixl_meta_dlist_t &remote,
        const std::string &remote_agent, nixlBackendReqH*&handle, const nixl_opt_b_args_t* opt_args)
{
    size_t lidx, ridx;
    ...
    if (des_cnt != remote.descCount()) { return NIXL_ERR_INVALID_PARAM; }
    ...
    for (int i = 0; i < des_cnt; i++) {
        ...
        if (!req->dlMatrix[lidx][ridx].in_use) {
            ...
            req->dlMatrix[lidx][ridx].ldescs =
                    new nixl_meta_dlist_t(local.getType(), local.isSorted());

            req->dlMatrix[lidx][ridx].rdescs =
                    new nixl_meta_dlist_t(remote.getType(), remote.isSorted());
        }
    ...
    for (lidx = 0; lidx < req->dlMatrix.size(); lidx++) {
        for (ridx = 0; ridx < req->dlMatrix[lidx].size(); ridx++) {
            ...
            ret = engines[lidx]->prepXfer(operation, *req->dlMatrix[lidx][ridx].ldescs,
                    *req->dlMatrix[lidx][ridx].rdescs, getEngName(remote_agent, ridx),
                    req->dlMatrix[lidx][ridx].ucx_req);
            if (NIXL_SUCCESS != ret) { goto err_clean_sub_req; }
        }
    }
...
// Data transfer
nixl_status_t nixlUcxMoEngine::postXfer(const nixl_xfer_op_t &operation,
        const nixl_meta_dlist_t &local, const nixl_meta_dlist_t &remote,
        const std::string &remote_agent, nixlBackendReqH*&handle, const nixl_opt_b_args_t* opt_args)
{
    ...
    for (size_t lidx = 0; lidx < req->dlMatrix.size(); lidx++) {
        for (size_t ridx = 0; ridx < req->dlMatrix[lidx].size(); ridx++) {
            ...
            ret = engines[lidx]->postXfer(operation, *req->dlMatrix[lidx][ridx].ldescs,
                    *req->dlMatrix[lidx][ridx].rdescs, getEngName(remote_agent, ridx),
                    req->dlMatrix[lidx][ridx].ucx_req);
...
nixl_status_t nixlUcxMoEngine::checkXfer(nixlBackendReqH* handle)
{
    nixlUcxMoRequestH* req = (nixlUcxMoRequestH*)handle;
...
nixl_status_t nixlUcxMoEngine::releaseReqH(nixlBackendReqH* handle)
{
    nixlUcxMoRequestH* req = (nixlUcxMoRequestH*)handle;
    nixl_status_t      out_ret = NIXL_SUCCESS;
...
int nixlUcxMoEngine::progress()
{
    int ret = 0;
    // Iterate over all elements cancelling each one
    for (auto &e : engines) { ret += e->progress(); }
    return ret;
}

nixl_status_t nixlUcxMoEngine::getNotifs(notif_list_t &notif_list)
{
    return engines[0]->getNotifs(notif_list);
}

nixl_status_t nixlUcxMoEngine::genNotif(const string &remote_agent, const string &msg)
{
