...
private:
    ...
    std::unordered_map<std::string, nixlUcxConnection, std::hash<std::string>, strEqual>
            remoteConnMap;
    ...
    bool isProgressThread() { return (std::this_thread::get_id() == pthr.get_id()); }

    // Request management
    static void _requestInit(void* request);
    static void _requestFini(void* request);
    void        requestReset(nixlUcxBckndReq* req) { _requestInit((void*)req); }

    // Connection helper
    static ucs_status_t connectionCheckAmCb(void* arg, const void* header, size_t header_length,
            void* data, size_t length, const ucp_am_recv_param_t* param);

    static ucs_status_t connectionTermAmCb(void* arg, const void* header, size_t header_length,
            void* data, size_t length, const ucp_am_recv_param_t* param);
    ...
    // Notifications
    static ucs_status_t notifAmCb(void* arg, const void* header, size_t header_length, void* data,
            size_t length, const ucp_am_recv_param_t* param);
    nixl_status_t       notifSendPriv(
...
public:
    ...
    nixl_status_t loadRemoteConnInfo(
    ...
    nixl_status_t prepXfer(const nixl_xfer_op_t &operation, const nixl_meta_dlist_t &local,
            ...
            nixlBackendReqH*&handle, const nixl_opt_b_args_t* opt_args = nullptr);
