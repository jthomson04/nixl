    ...
    : nixlBackendEngine(init_params)
{
    ...
    uc = new nixlUcxContext(
            devs, sizeof(nixlUcxBckndReq), _requestInit, _requestFini, NIXL_UCX_MT_WORKER);
...
nixl_status_t nixlUcxEngine::endConn(const std::string &remote_agent)
{
    ...
    if (uw->disconnect_nb(conn.ep) < 0) { return NIXL_ERR_BACKEND; }
...
nixl_status_t nixlUcxEngine::connect(const std::string &remote_agent)
{
    ...
    ret = uw->sendAm(conn.ep, CONN_CHECK, &hdr, sizeof(struct nixl_ucx_am_hdr),
...
nixl_status_t nixlUcxEngine::disconnect(const std::string &remote_agent)
{
    ...
    if (remote_agent != localAgent) {
        ...
        ret = uw->sendAm(conn.ep, DISCONNECT, &hdr, sizeof(struct nixl_ucx_am_hdr),
                (void*)localAgent.data(), localAgent.size(), flags, req);
...
nixl_status_t nixlUcxEngine::retHelper(nixl_status_t ret, nixlUcxBckndReq* head, nixlUcxReq &req)
{
    ...
    switch (ret) {
        ...
        default:
            ...
            if (head->next()) { releaseReqH(head->next()); }
            return NIXL_ERR_BACKEND;
    }
    return NIXL_SUCCESS;
}
        ...
        const std::string &remote_agent, nixlBackendReqH*&handle, const nixl_opt_b_args_t* opt_args)
{
    ...
    for (i = 0; i < lcnt; i++) {
        ...
        switch (operation) {
            case NIXL_READ:
                ret = uw->read(
                        rmd->conn.ep, (uint64_t)raddr, rmd->rkey, laddr, lmd->mem, lsize, req);
                break;
            case NIXL_WRITE:
                ret = uw->write(
                        rmd->conn.ep, laddr, lmd->mem, (uint64_t)raddr, rmd->rkey, lsize, req);
                break;
            default:
                return NIXL_ERR_INVALID_PARAM;
        }

        if (retHelper(ret, head, req)) { return ret; }
    }
...
nixl_status_t nixlUcxEngine::releaseReqH(nixlBackendReqH* handle)
{
    ...
    if (head->next() || !head->is_complete()) {
        ...
        while (head) {
            ...
            head = req->unlink();
            requestReset(req);
            if (!done) { uw->reqCancel((nixlUcxReq)req); }
            uw->reqRelease((nixlUcxReq)req);
        }
    } else {
        ...
        const std::string &remote_agent, const std::string &msg, nixlUcxReq &req)
{
    ...
    ret = uw->sendAm(conn.ep, NOTIF_STR, &hdr, sizeof(struct nixl_ucx_am_hdr),
            (void*)ser_msg->data(), ser_msg->size(), flags, req);
