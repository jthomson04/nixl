...
nixl_status_t gdsUtil::registerFileHandle(
        int fd, size_t size, std::string metaInfo, gdsFileHandle &gds_handle)
{
...
nixlGdsIOBatch::nixlGdsIOBatch(unsigned int size) : max_reqs(size)
{
    CUfileError_t err;
...
nixl_status_t nixlGdsIOBatch::addToBatch(CUfileHandle_t fh, void* buffer, size_t size,
        size_t file_offset, size_t ptr_offset, CUfileOpcode_t type)
{
    CUfileIOParams_t* params = nullptr;

    if (batch_size >= max_reqs) return NIXL_ERR_BACKEND;
...
nixl_status_t nixlGdsIOBatch::checkStatus()
{
    ...
    errBatch = cuFileBatchIOGetStatus(batch_handle, nr, &nr, io_batch_events, NULL);
    if (errBatch.err != 0) {
