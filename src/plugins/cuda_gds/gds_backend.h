...
public:
    ...
    GdsTransferRequestH(
            void* a, size_t s, size_t offset, CUfileHandle_t handle, CUfileOpcode_t operation)
    {
        addr = a;
        size = s;
        file_offset = offset;
...
public:
    ...
    ~nixlGdsBackendReqH()
    {
        for (auto* batch : batch_io_list) { delete batch; }
        batch_io_list.clear();
    }
};
...
private:
    ...
    nixl_status_t   createBatches(const nixl_xfer_op_t &operation, const nixl_meta_dlist_t &local,
              const nixl_meta_dlist_t &remote, nixlGdsBackendReqH* gds_handle);
