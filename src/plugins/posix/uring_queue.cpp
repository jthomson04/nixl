...
nixl_status_t UringQueue::init(int entries, const io_uring_params &params, bool read_op)
{
    ...
    NIXL_INFO << "io_uring features:" << " SQPOLL="
              << ((mutable_params.features & IORING_FEAT_SQPOLL_NONFIXED) ? "yes" : "no")
...
UringQueue::UringQueue(int num_entries, const io_uring_params &params, bool is_read)
    : num_entries(num_entries), num_completed(0), is_read(is_read),
      prep_op(is_read ? reinterpret_cast<io_uring_prep_func_t>(io_uring_prep_read)
                      : reinterpret_cast<io_uring_prep_func_t>(io_uring_prep_write))
{
    ...
    if (status != NIXL_SUCCESS) { throw std::runtime_error("Failed to initialize UringQueue"); }
}
...
nixl_status_t UringQueue::checkCompleted()
{
    ...
    if (num_completed % (num_entries / 10) == 0) {
        NIXL_DEBUG << "Queue progress: " << (num_completed * 100.0 / num_entries) << "% complete";
    }
