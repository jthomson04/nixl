...
{
public:
    static std::unique_ptr<nixlPosixQueue> createQueue(
            bool use_aio, int num_entries, bool is_read = false, const void* params = nullptr);
};
...
public:
    nixlPosixBackendReqH(const nixl_xfer_op_t &operation, const nixl_meta_dlist_t &local,
            const nixl_meta_dlist_t &remote, const nixl_opt_b_args_t* opt_args,
...
private:
    ...
    bool validatePrepXferParams(const nixl_xfer_op_t &operation, const nixl_meta_dlist_t &local,
...
public:
    ...
    nixl_status_t registerMem(
            const nixlBlobDesc &mem, const nixl_mem_t &nixl_mem, nixlBackendMD*&out) override;
