...
#ifdef HAVE_LIBURING
    static std::unique_ptr<nixlPosixQueue> createUringQueue(
            int num_entries, bool is_read, const io_uring_params* params)
    {
...
#else
    static std::unique_ptr<nixlPosixQueue> createUringQueue(
            int num_entries, bool is_read, const void* params)
    {
        (void)num_entries;  // Avoid unused parameter warning
        (void)is_read;
        (void)params;
