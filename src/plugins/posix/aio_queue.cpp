...
nixl_status_t aioQueue::submit() {
    ...
    for (auto& aiocb : aiocbs) {
        ...
        if (ret < 0) {
            ...
            NIXL_ERROR << "AIO submit failed: " << nixl_strerror(errno);
            return NIXL_ERR_BACKEND;
        }
    ...
    for (auto& aiocb : aiocbs) {
        ...
        if (status == 0) {  // Operation completed
            ...
            if (ret < 0 || ret != static_cast<ssize_t>(aiocb.aio_nbytes)) {
                NIXL_ERROR << "AIO operation failed or incomplete: " << nixl_strerror(errno);
                return NIXL_ERR_BACKEND;
            }
            num_completed++;
            ...
            NIXL_ERROR << "AIO error: " << nixl_strerror(status);
            return NIXL_ERR_BACKEND;
        }
    }
