...
{
public:
    static bool isAioAvailable() { return QueueFactory::isAioAvailable(); }

    static bool isUringAvailable() { return QueueFactory::isUringAvailable(); }
        ...
        if (custom_params) {
            ...
            if (custom_params->count("use_uring") > 0) {
                ...
                if (value == "true" || value == "1") {
                    ...
                    NIXL_ERROR << "io_uring backend requested but not available - not built with "
                                  "liburing support";
                    return {NIXL_ERR_NOT_SUPPORTED, false};
#endif
...
nixlPosixBackendReqH::nixlPosixBackendReqH(const nixl_xfer_op_t &op, const nixl_meta_dlist_t &loc,
        const nixl_meta_dlist_t &rem, const nixl_opt_b_args_t* args, const nixl_b_params_t* params)
    : operation(op), local(loc), remote(rem), opt_args(args), custom_params_(params),
      queue_depth_(loc.descCount()), is_prepped(false), status(NIXL_SUCCESS)
{
    ...
    try {
        ...
        if (status != NIXL_SUCCESS) {
            throw std::runtime_error("Failed to initialize queues: " + std::to_string(status));
        }
...
nixl_status_t nixlPosixBackendReqH::initQueues(bool use_aio)
{
    try {
        if (use_aio) {
            ...
            if (!queue) { throw std::runtime_error("Failed to create AIO queue"); }
            ...
            if (!queue) { throw std::runtime_error("Failed to create io_uring queue"); }
...
{
    if (!queue) { return NIXL_ERR_BACKEND; }

    if (is_prepped) { return status; }

    const int desc_count = static_cast<int>(local.descCount());
    for (int i = 0; i < desc_count; ++i) {
        status = queue->prepareIO(remote[i].devId, reinterpret_cast<void*>(local[i].addr),
                remote[i].len, remote[i].addr);
...
{
    if (initErr) { return NIXL_ERR_BACKEND; }
    return NIXL_SUCCESS;
}
...
bool nixlPosixEngine::validatePrepXferParams(const nixl_xfer_op_t &operation,
        ...
        const std::string &remote_agent, const std::string &local_agent)
{
    if (remote_agent != local_agent) {
        NIXL_ERROR << absl::StrFormat(
                "Error: Remote agent must match the requesting agent (%s). Got %s", local_agent,
                remote_agent);
        return false;
    }
    ...
    if (local.getType() != DRAM_SEG) {
        NIXL_ERROR << absl::StrFormat(
                "Error: Local memory type must be DRAM_SEG, got %d", local.getType());
        return false;
    }

    if (remote.getType() != FILE_SEG) {
        NIXL_ERROR << absl::StrFormat(
                "Error: Remote memory type must be FILE_SEG, got %d", remote.getType());
        return false;
    }

    if (local.descCount() != remote.descCount()) {
        NIXL_ERROR << absl::StrFormat(
                "Error: Mismatch in descriptor counts - local: %d, remote: %d", local.descCount(),
                remote.descCount());
        return false;
    }
...
nixl_status_t nixlPosixEngine::registerMem(
