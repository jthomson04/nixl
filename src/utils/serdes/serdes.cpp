...
/* Serialization/Deserialization for Byte buffers */
...
ssize_t nixlSerDes::getBufLen(const std::string &tag) const
{
    ...
    _stringToBytes(
            &len, workingStr.substr(des_offset + tag.size(), sizeof(ssize_t)), sizeof(ssize_t));
...
/* Serialization/Deserialization buffer management */
