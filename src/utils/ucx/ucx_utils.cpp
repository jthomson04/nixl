...
{
    if (status == UCS_OK) [[likely]] { return NIXL_SUCCESS; }
...
nixl_status_t nixlUcxEp::connect(ucp_worker_h worker, void* addr)
{
    ...
    if (status == NIXL_SUCCESS) { setState(NIXL_UCX_EP_STATE_CONNECTED); }

    return status;
}

nixl_status_t nixlUcxEp::closeImpl(ucp_worker_h worker, ucp_ep_close_flags_t flags)
{
...
nixlUcxContext::nixlUcxContext(std::vector<std::string> devs, size_t req_size,
        nixlUcxContext::req_cb_t init_cb, nixlUcxContext::req_cb_t fini_cb, nixl_ucx_mt_t __mt_type)
{
    ...
    if (devs.size()) {
        ...
        string       dev_str = "";
        unsigned int i;
        for (i = 0; i < devs.size() - 1; i++) { dev_str = dev_str + devs[i] + ":1,"; }
...
int nixlUcxWorker::epAddr(uint64_t &addr, size_t &size)
{
    ...
    wattr.field_mask = UCP_WORKER_ATTR_FIELD_ADDRESS | UCP_WORKER_ATTR_FIELD_ADDRESS_FLAGS;
...
nixl_status_t nixlUcxWorker::sendAm(nixlUcxEp &ep, unsigned msg_id, void* hdr, size_t hdr_len,
        void* buffer, size_t len, uint32_t flags, nixlUcxReq &req)
{
    ucs_status_ptr_t    request;
...
int nixlUcxWorker::getRndvData(void* data_desc, void* buffer, size_t len,
...
nixl_status_t nixlUcxWorker::read(nixlUcxEp &ep, uint64_t raddr, nixlUcxRkey &rk, void* laddr,
        nixlUcxMem &mem, size_t size, nixlUcxReq &req)
{
    ...
    ucs_status_ptr_t request = ucp_get_nbx(ep.getHandle(), laddr, size, raddr, rk.rkeyh, &param);
    if (UCS_PTR_IS_PTR(request)) {
        req = (void*)request;
        return NIXL_IN_PROG;
    }
...
nixl_status_t nixlUcxWorker::write(nixlUcxEp &ep, void* laddr, nixlUcxMem &mem, uint64_t raddr,
        nixlUcxRkey &rk, size_t size, nixlUcxReq &req)
{
    nixl_status_t status = ep.checkTxState();
    if (status != NIXL_SUCCESS) { return status; }
    ...
    ucs_status_ptr_t request = ucp_put_nbx(ep.getHandle(), laddr, size, raddr, rk.rkeyh, &param);
    if (UCS_PTR_IS_PTR(request)) {
        req = (void*)request;
        return NIXL_IN_PROG;
    }
...
{
    if (req == NULL) { return NIXL_SUCCESS; }
