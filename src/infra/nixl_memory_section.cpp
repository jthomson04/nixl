...
nixl_reg_dlist_t nixlLocalSection::getStringDesc(
        const nixlBackendEngine* backend, const nixl_meta_dlist_t &d_list) const
{
    nixl_status_t    ret;
    nixlBlobDesc     element;
    nixlBasicDesc*   p = &element;
    nixl_reg_dlist_t output_desclist(d_list.getType(), d_list.isSorted());
...
nixl_status_t nixlLocalSection::addDescList(const nixl_reg_dlist_t &mem_elms,
        nixlBackendEngine* backend, nixl_meta_dlist_t &remote_self)
{
    ...
    for (int i = 0; i < mem_elms.descCount(); ++i) {
        ...
        if ((ret1 != NIXL_SUCCESS) || (ret2 != NIXL_SUCCESS)) {
            for (int j = 0; j < i; ++j) {
                ...
                backend->deregisterMem((*(const nixl_meta_dlist_t*)target)[index].metadataP);
...
nixl_status_t nixlLocalSection::remDescList(
        const nixl_meta_dlist_t &mem_elms, nixlBackendEngine* backend)
{
    if (!backend) return NIXL_ERR_INVALID_PARAM;
    nixl_mem_t    nixl_mem = mem_elms.getType();
    section_key_t sec_key = std::make_pair(nixl_mem, backend);
    ...
    for (auto &elm : mem_elms) {
        ...
        if (index < 0) return NIXL_ERR_UNKNOWN;
...
nixlLocalSection::~nixlLocalSection()
{
    ...
    for (auto &seg : sectionMap) {
        eng = seg.first.second;
        m_desc = seg.second;
        for (auto &elm : *m_desc) eng->deregisterMem(elm.metadataP);
        delete m_desc;
    }
        ...
        const nixl_reg_dlist_t &mem_elms, nixlBackendEngine* backend)
{
    ...
    nixl_mem_t    nixl_mem = mem_elms.getType();
    section_key_t sec_key = std::make_pair(nixl_mem, backend);
    if (sectionMap.count(sec_key) == 0) sectionMap[sec_key] = new nixl_meta_dlist_t(nixl_mem, true);
...
nixlRemoteSection::~nixlRemoteSection()
{
    ...
    for (auto &seg : sectionMap) {
        eng = seg.first.second;
        m_desc = seg.second;
        for (auto &elm : *m_desc) eng->unloadMD(elm.metadataP);
        delete m_desc;
    }
    // nixlMemSection destructor will clean up the rest
}
