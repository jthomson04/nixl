...
nixl_status_t nixlMemSection::populate(
        const nixl_xfer_dlist_t &query, nixlBackendEngine* backend, nixl_meta_dlist_t &resp) const
{
    if (query.getType() != resp.getType()) return NIXL_ERR_INVALID_PARAM;
    // 1-to-1 mapping cannot hold
    if (query.isSorted() != resp.isSorted()) return NIXL_ERR_INVALID_PARAM;
    ...
    if (!base->isSorted()) {
        ...
        if (q_sorted) {
            ...
            for (int i = 0; i < query.descCount(); ++i) {
                found = false;
                q = &query[i];
                auto itr = std::lower_bound(base->begin() + last_found, base->end(), *q);
...
nixl_status_t nixlLocalSection::addDescList(
        const nixl_reg_dlist_t &mem_elms, nixlBackendEngine* backend, nixl_sec_dlist_t &remote_self)
{
...
nixl_status_t nixlLocalSection::remDescList(
        const nixl_reg_dlist_t &mem_elms, nixlBackendEngine* backend)
{
    if (!backend) return NIXL_ERR_INVALID_PARAM;
    nixl_mem_t    nixl_mem = mem_elms.getType();
    section_key_t sec_key = std::make_pair(nixl_mem, backend);
...
namespace
{
nixl_status_t serializeSections(nixlSerDes* serializer, const section_map_t &sections)
{
    nixl_status_t ret;
...
nixl_status_t nixlLocalSection::serializePartial(nixlSerDes* serializer,
        const backend_set_t &backends, const nixl_reg_dlist_t &mem_elms) const
{
    nixl_mem_t    nixl_mem = mem_elms.getType();
    ...
    for (auto &[sec_key, m_desc] : mem_elms_to_serialize) delete m_desc;
    return ret;
}
...
nixlLocalSection::~nixlLocalSection()
{
    for (auto &[sec_key, dlist] : sectionMap) {
        nixlBackendEngine* eng = sec_key.second;
        for (auto &elm : *dlist) eng->deregisterMem(elm.metadataP);
        delete dlist;
    }
        ...
        const nixl_reg_dlist_t &mem_elms, nixlBackendEngine* backend)
{
    ...
    nixl_mem_t    nixl_mem = mem_elms.getType();
    section_key_t sec_key = std::make_pair(nixl_mem, backend);
    if (sectionMap.count(sec_key) == 0) sectionMap[sec_key] = new nixl_sec_dlist_t(nixl_mem, true);
...
nixl_status_t nixlRemoteSection::loadRemoteData(
        nixlSerDes* deserializer, backend_map_t &backendToEngineMap)
{
    nixl_status_t  ret;
    size_t         seg_count;
    ...
    for (size_t i = 0; i < seg_count; ++i) {
        ...
        if (nixl_backend.size() == 0) return NIXL_ERR_INVALID_PARAM;
...
nixl_status_t nixlRemoteSection::loadLocalData(
        const nixl_sec_dlist_t &mem_elms, nixlBackendEngine* backend)
{
    ...
    for (auto &elm : mem_elms) target->addDesc(elm);
...
nixlRemoteSection::~nixlRemoteSection()
{
    for (auto &[sec_key, dlist] : sectionMap) {
        nixlBackendEngine* eng = sec_key.second;
        for (auto &elm : *dlist) eng->unloadMD(elm.metadataP);
        delete dlist;
    }
    // nixlMemSection destructor will clean up the rest
}
