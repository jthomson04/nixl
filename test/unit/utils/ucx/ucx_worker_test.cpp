...
void completeRequest(
        nixlUcxWorker w[2], std::string op, bool is_flush, nixl_status_t ret, nixlUcxReq &req)
{
    ...
    if (ret == NIXL_SUCCESS) {
        ...
        if (!is_flush) { cout << "NOTE: Testing non-inline " << op << " path!" << endl; }
...
int main()
{
    ...
    nixlUcxContext c[2] = {
            ...
            nixlUcxContext(
                    devs, sizeof(requestData), nixlUcxRequestInit, NULL, NIXL_UCX_MT_SINGLE)};
...
#ifdef USE_VRAM
    checkCudaError(
            cudaMemcpy(chk_buffer, buffer[1], 128, cudaMemcpyDeviceToHost), "Failed to memcpy");
#else
    memcpy(chk_buffer, buffer[1], buf_size);
#endif

    for (i = 0; i < buf_size / 2; i++) { assert(chk_buffer[i] == 0xda); }
    for (; i < buf_size; i++) { assert(chk_buffer[i] == 0xbb); }
    ...
    for (i = 0; i < buf_size / 3; i++) { assert(chk_buffer[i] == 0xbb); }
    for (; i < buf_size; i++) { assert(chk_buffer[i] == 0xda); }
