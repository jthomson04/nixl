...
ucs_status_t check_buffer(void* arg, const void* header, size_t header_length, void* data,
...
ucs_status_t rndv_test(void* arg, const void* header, size_t header_length, void* data,
        size_t length, const ucp_am_recv_param_t* param)
{
    struct sample_header* hdr = (struct sample_header*)header;
    ...
    while (ret == 0) { ret = am_worker->test(req); }
...
int main()
{
    ...
    nixlUcxContext c[2] = {nixlUcxContext(devs, 0, NULL, NULL, NIXL_UCX_MT_SINGLE),
    ...
    /* Test first callback */
    ret = w[1].sendAm(ep[1], check_cb_id, &hdr, sizeof(struct sample_header), (void*)&buffer,
            sizeof(buffer), 0, req);
    assert(ret == 0);
    ...
    ret = w[1].sendAm(
            ep[1], rndv_cb_id, &hdr, sizeof(struct sample_header), big_buffer, 8192, flags, req);
    assert(ret == 0);
    ...
    /* Test shutdown */
    for (i = 0; i < 2; i++) { assert(0 == w[i].disconnect(ep[i])); }
