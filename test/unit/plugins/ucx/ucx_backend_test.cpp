...
public:
    ...
    bool needRelease() { return release; }

    void isLast()
    {
        if (reuse) { release = true; }
    }
...
void allocateAndRegister(nixlBackendEngine* ucx, int dev_id, nixl_mem_t mem_type, void*&addr,
        size_t len, nixlBackendMD*&md)
{
    nixlBlobDesc desc;
...
void deallocateAndDeregister(
        nixlBackendEngine* ucx, int dev_id, nixl_mem_t mem_type, void*&addr, nixlBackendMD*&md)
{
    ucx->deregisterMem(md);
    releaseBuffer(mem_type, dev_id, addr);
}

void loadRemote(nixlBackendEngine* ucx, int dev_id, std::string agent, nixl_mem_t mem_type,
        void* addr, size_t len, nixlBackendMD*&lmd, nixlBackendMD*&rmd)
{
    nixlBlobDesc info;
...
void populateDescs(nixl_meta_dlist_t &descs, int dev_id, void* addr, int desc_cnt, size_t desc_size,
        nixlBackendMD*&md)
{
...
void performTransfer(nixlBackendEngine* ucx1, nixlBackendEngine* ucx2,
        nixl_meta_dlist_t &req_src_descs, nixl_meta_dlist_t &req_dst_descs, void* addr1,
        void* addr2, size_t len, nixl_xfer_op_t op, testHndlIterator &hiter, bool progress,
        bool use_notif)
{
    int           ret2;
    nixl_status_t ret3;
    ...
    for (size_t i = 0; i < len; i++) { assert(((uint8_t*)chkptr1)[i] == ((uint8_t*)chkptr2)[i]); }
    ...
    for (size_t i = 0; i < sizeof(ops) / sizeof(ops[i]); i++) {
        for (bool use_notif : use_notifs) {
            ...
            for (int k = 0; k < iter; k++) {
                ...
                performTransfer(ucx, ucx, req_src_descs, req_dst_descs, addr1, addr2, len, ops[i],
                        hiter, p_thread, use_notif);
            }
        }
    }
...
void test_inter_agent_transfer(bool p_thread, bool reuse_hndl, nixlBackendEngine* ucx1,
        nixl_mem_t src_mem_type, int src_dev_id, nixlBackendEngine* ucx2, nixl_mem_t dst_mem_type,
        int dst_dev_id)
{
    int ret;
    int iter = 10;
    ...
    for (size_t i = 0; i < sizeof(ops) / sizeof(ops[i]); i++) {
        for (bool use_notif : use_notifs) {
            ...
            for (int k = 0; k < iter; k++) {
                ...
                performTransfer(ucx1, ucx2, req_src_descs, req_dst_descs, addr1, addr2, len, ops[i],
                        hiter, !p_thread, use_notif);
            }
        }
    }
...
#ifdef HAVE_CUDA
        if (n_vram_dev > 0) { test_intra_agent_transfer(thread_on[i], ucx[i][0], VRAM_SEG); }
#endif
    }

    for (int i = 0; i < 2; i++) {
        test_inter_agent_transfer(
                thread_on[i], false, ucx[i][0], DRAM_SEG, 0, ucx[i][1], DRAM_SEG, 0);
        test_inter_agent_transfer(
                thread_on[i], true, ucx[i][0], DRAM_SEG, 0, ucx[i][1], DRAM_SEG, 0);

#ifdef HAVE_CUDA
        if (n_vram_dev > 1) {
            test_inter_agent_transfer(thread_on[i], false, ucx[i][0], VRAM_SEG, dev_ids[0],
                    ucx[i][1], VRAM_SEG, dev_ids[1]);
            test_inter_agent_transfer(thread_on[i], true, ucx[i][0], VRAM_SEG, dev_ids[0],
                    ucx[i][1], VRAM_SEG, dev_ids[1]);
            test_inter_agent_transfer(thread_on[i], true, ucx[i][0], DRAM_SEG, dev_ids[0],
                    ucx[i][1], VRAM_SEG, dev_ids[1]);
            test_inter_agent_transfer(thread_on[i], true, ucx[i][0], VRAM_SEG, dev_ids[0],
