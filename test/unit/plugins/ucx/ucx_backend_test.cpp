...
void performTransfer(nixlBackendEngine* ucx1, nixlBackendEngine* ucx2,
        ...
        void* addr2, size_t len, nixl_xfer_op_t op, bool progress, bool use_notif)
{
    int              ret2;
    nixl_status_t    ret3;
...
void test_inter_agent_transfer(bool p_thread, nixlBackendEngine* ucx1, nixl_mem_t src_mem_type,
        int src_dev_id, nixlBackendEngine* ucx2, nixl_mem_t dst_mem_type, int dst_dev_id)
{
    int ret;
    int iter = 10;
    ...
    std::cout << std::endl << std::endl;
    std::cout << "****************************************************" << std::endl;
    std::cout << "    Inter-agent memory transfer test P-Thr=" << (p_thread ? "ON" : "OFF")
              << std::endl;
...
int main()
{
    ...
    for (int i = 0; i < 2; i++) {
        test_inter_agent_transfer(thread_on[i], ucx[i][0], DRAM_SEG, 0, ucx[i][1], DRAM_SEG, 0);
#ifdef HAVE_CUDA
        if (n_vram_dev > 1) {
            test_inter_agent_transfer(
                    thread_on[i], ucx[i][0], VRAM_SEG, dev_ids[0], ucx[i][1], VRAM_SEG, dev_ids[1]);
            test_inter_agent_transfer(
                    thread_on[i], ucx[i][0], DRAM_SEG, dev_ids[0], ucx[i][1], VRAM_SEG, dev_ids[1]);
            test_inter_agent_transfer(
                    thread_on[i], ucx[i][0], VRAM_SEG, dev_ids[0], ucx[i][1], DRAM_SEG, dev_ids[1]);
        }
#endif
    }
