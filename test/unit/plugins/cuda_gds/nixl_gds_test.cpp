...
void print_usage(const char* program_name)
{
    std::cerr << "Usage: " << program_name << " [options] <directory_path>\n"
              ...
              << "  -b, --batch-limit SIZE  Maximum requests per batch  (default: 128, Max "
                 "allowed: 1-128)\n"
              << "  -m, --max-req-size SIZE Maximum size per request (default: 16M, Max allowed: "
                 "16M)\n"
              ...
              << "\nExample:\n"
              << "  " << program_name
              << " -d -n 100 -s 2M -p 16 -b 256 -m 32M -t 5 -D /path/to/dir\n";
}
...
cudaError_t fill_gpu_test_pattern(void* gpu_buffer, size_t size)
{
    ...
    if (!host_buffer) { return cudaErrorMemoryAllocation; }
...
int main(int argc, char* argv[])
{
    ...
    static struct option long_options[] = {{"dram", no_argument, 0, 'd'},
            {"vram", no_argument, 0, 'v'}, {"num-transfers", required_argument, 0, 'n'},
            {"size", required_argument, 0, 's'}, {"no-read", no_argument, 0, 'r'},
            {"no-write", no_argument, 0, 'w'}, {"pool-size", required_argument, 0, 'p'},
            {"batch-limit", required_argument, 0, 'b'}, {"max-req-size", required_argument, 0, 'm'},
            {"iterations", required_argument, 0, 't'}, {"direct", no_argument, 0, 'D'},
    ...
    if (!use_dram && !use_vram) { use_vram = true; }
    ...
    if (use_vram) { vram_addr = new void*[num_transfers]; }
    if (use_dram) { dram_addr = new void*[num_transfers]; }
    ...
    std::cout << "\n============================================================" << std::endl;
    std::cout << "                 NIXL STORAGE TEST STARTING (GDS PLUGIN)                     "
              << std::endl;
    std::cout << "============================================================" << std::endl;
    ...
    if (!skip_write) {
        ...
        for (unsigned int iter = 0; iter < iterations; iter++) {
            ...
            if (iterations > 1) { printProgress(float(iter + 1) / iterations); }
        }
    ...
    if (!skip_read && !skip_write) {
        ...
        for (i = 0; i < num_transfers; i++) {
            ...
            if (use_dram && dram_addr[i]) { clear_buffer(dram_addr[i], transfer_size); }
            printProgress(float(i + 1) / num_transfers);
        }
    }
