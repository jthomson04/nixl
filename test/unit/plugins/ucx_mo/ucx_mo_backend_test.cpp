...
void createLocalDescs(nixlBackendEngine* ucx, nixl_meta_dlist_t &descs, int dev_cnt,
        dev_distr_t dist_f, int desc_cnt, size_t desc_size)
{
...
void destroyLocalDescs(nixlBackendEngine* ucx, nixl_meta_dlist_t &descs)
{
    ...
    while (descs.descCount()) { descs.remDesc(0); }
}

void createRemoteDescs(nixlBackendEngine* src_ucx, std::string agent, nixl_meta_dlist_t &src_descs,
        nixlBackendEngine* dst_ucx, nixl_meta_dlist_t &dst_descs)
{
    ...
    for (int i = 0; i < src_descs.descCount(); i++) {
        ...
        *((nixlBasicDesc*)&desc_m) = (nixlBasicDesc)src_descs[i];
        ...
        if (is_local) {
            ...
            status = dst_ucx->loadRemoteMD(desc_s, src_descs.getType(), agent, desc_m.metadataP);
        }
...
void destroyRemoteDescs(nixlBackendEngine* dst_ucx, nixl_meta_dlist_t &dst_descs)
{
    nixl_status_t status;
    ...
    while (dst_descs.descCount()) { dst_descs.remDesc(0); }
}

void performTransfer(nixlBackendEngine* ucx1, nixlBackendEngine* ucx2,
        nixl_meta_dlist_t &req_src_descs, nixl_meta_dlist_t &req_dst_descs, nixl_xfer_op_t op,
...
void test_agent_transfer(bool p_thread, nixlBackendEngine* ucx1, nixl_mem_t src_mem_type,
        int src_dev_cnt, dev_distr_t src_dist_f, nixlBackendEngine* ucx2, nixl_mem_t dst_mem_type,
        int dst_dev_cnt, dev_distr_t dst_dist_f)
{
    int           iter = 10;
    nixl_status_t status;
    ...
    std::cout << ((is_local) ? std::string("IntrA") : std::string("IntEr"))
              << "-agent memory transfer test P-Thr=" << (p_thread ? "ON" : "OFF") << std::endl;
    ...
    createLocalDescs(ucx1, ucx1_src_descs, src_dev_cnt, src_dist_f, desc_cnt, desc_size);
    createLocalDescs(ucx2, ucx2_src_descs, dst_dev_cnt, dst_dist_f, desc_cnt, desc_size);
    createRemoteDescs(ucx2, agent2, ucx2_src_descs, ucx1, ucx1_dst_descs);
    ...
    for (size_t i = 0; i < sizeof(ops) / sizeof(ops[i]); i++) {
        for (bool use_notif : use_notifs) {
            ...
            for (int k = 0; k < iter; k++) {
                ...
                /* Test */
                performTransfer(
                        ucx1, ucx2, ucx1_src_descs, ucx1_dst_descs, ops[i], !p_thread, use_notif);
            }
        }
    }
...
#define NUM_WORKERS 8
    ...
    for (size_t i = 0; i < THREAD_ON_SIZE; i++) {
        ...
        test_agent_transfer(thread_on[i], ucx[i][0], DRAM_SEG, ndevices, dev_distr_rr, ucx[i][0],
                DRAM_SEG, ndevices, dev_distr_blk);
#ifdef HAVE_CUDA
        if (n_vram_dev) {
            test_agent_transfer(thread_on[i], ucx[i][0], VRAM_SEG, ndevices, dev_distr_rr,
    ...
    for (size_t i = 0; i < THREAD_ON_SIZE; i++) {
        test_agent_transfer(thread_on[i], ucx[i][0], DRAM_SEG, ndevices, dev_distr_rr, ucx[i][1],
                DRAM_SEG, ndevices, dev_distr_blk);
        ...
        if (n_vram_dev) {
            ...
            test_agent_transfer(thread_on[i], ucx[i][0], DRAM_SEG, n_vram_dev, dev_distr_rr,
                    ucx[i][1], VRAM_SEG, n_vram_dev, dev_distr_blk);
        }
#endif
    }
    ...
    // Allocate UCX engines
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 2; j++) { releaseEngine(ucx[i][j]); }
    }
}
