...
const PluginDesc ucx_plugin_desc{.name = "UCX", .type = PluginDesc::PluginType::Real};
const PluginDesc gds_plugin_desc{.name = "GDS", .type = PluginDesc::PluginType::Real};
const PluginDesc ucx_mo_plugin_desc{.name = "UCX_MO", .type = PluginDesc::PluginType::Real};

class LoadSinglePluginTestFixture : public testing::TestWithParam<PluginDesc>
{
protected:
    ...
    {
#if !TEST_ALL_PLUGINS
        if (GetParam().type == PluginDesc::PluginType::Real) GTEST_SKIP();
#endif
    ...
    {
#if !TEST_ALL_PLUGINS
        if (GetParam().type == PluginDesc::PluginType::Real) return;
#endif
...
class LoadMultiplePluginsTestFixture : public testing::TestWithParam<std::vector<PluginDesc>>
{
protected:
    nixlPluginManager &plugin_manager_ = nixlPluginManager::getInstance();
...
#if !TEST_ALL_PLUGINS
            if (plugin.type == PluginDesc::PluginType::Real) continue;
#endif
    ...
    bool AreAllLoaded()
    {
        return all_of(plugin_handles_.begin(), plugin_handles_.end(),
...
protected:
    ...
    void TearDown() override
    {
        for (const auto &plugin : loaded_plugins_) plugin_manager_.unloadPlugin(plugin);
    }
    ...
    {
        if (prev_plugins_.find(name) != prev_plugins_.end()) return true;
        if (loaded_plugins_.find(name) != loaded_plugins_.end()) return false;
    ...
    {
        if (loaded_plugins_.find(name) == loaded_plugins_.end()) return;
...
INSTANTIATE_TEST_SUITE_P(MockLoadPluginInstantiation, LoadSinglePluginTestFixture,
        testing::Values(
                PluginDesc{.name = mock_basic_plugin_name, .type = PluginDesc::PluginType::Mock}));
INSTANTIATE_TEST_SUITE_P(
        UcxLoadPluginInstantiation, LoadSinglePluginTestFixture, testing::Values(ucx_plugin_desc));
INSTANTIATE_TEST_SUITE_P(
        GdsLoadPluginInstantiation, LoadSinglePluginTestFixture, testing::Values(gds_plugin_desc));
INSTANTIATE_TEST_SUITE_P(UcxMoLoadPluginInstantiation, LoadSinglePluginTestFixture,
        testing::Values(ucx_mo_plugin_desc));

/* Load multiple plugins tests instantiations. */
INSTANTIATE_TEST_SUITE_P(UcxGdsLoadMultiplePluginInstantiation, LoadMultiplePluginsTestFixture,
        testing::Values(std::vector<PluginDesc>{ucx_plugin_desc, gds_plugin_desc}));
INSTANTIATE_TEST_SUITE_P(UcxUcxMoLoadMultiplePluginInstantiation, LoadMultiplePluginsTestFixture,
        testing::Values(std::vector<PluginDesc>{ucx_plugin_desc, ucx_mo_plugin_desc}));
INSTANTIATE_TEST_SUITE_P(GdsUcxMoLoadMultiplePluginInstantiation, LoadMultiplePluginsTestFixture,
        testing::Values(std::vector<PluginDesc>{gds_plugin_desc, ucx_mo_plugin_desc}));
INSTANTIATE_TEST_SUITE_P(UcxGdsUcxMoLoadMultiplePluginInstantiation, LoadMultiplePluginsTestFixture,
        testing::Values(
                std::vector<PluginDesc>{ucx_plugin_desc, gds_plugin_desc, ucx_mo_plugin_desc}));
