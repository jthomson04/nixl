...
protected:
    ...
    {
        nixlAgentConfig cfg(
                false, false, 0, 0, 100000, nixl_thread_sync_t::NIXL_THREAD_SYNC_STRICT);
    ...
    void verifyTransfer(nixlAgent &agent, const nixl_opt_args_t &extra_params)
    {
        ...
        nixl_status_t status = agent.createXferReq(
                NIXL_WRITE, src_list, dst_list, "test_agent", xfer_req, &extra_params);
        EXPECT_EQ(status, NIXL_SUCCESS);
...
TEST_F(MultiThreadingTestFixture, ConcurrentTransfers)
{
    ...
    auto transfer_sequence = [&]() { verifyTransfer(agent, extra_params); };
