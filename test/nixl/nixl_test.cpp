...
int main(int argc, char* argv[])
{
    ...
    if (argc < 4) {
        ...
        std::cout << "<Role> "
                  << "Initiator IP> <Initiator Port>" << std::endl;
        exit(-1);
    }
    ...
    for (int i = 0; i < NUM_TRANSFERS; i++) {
        ...
        if (role != "target") {
            ...
            std::cout << "Allocating for initiator : " << addr[i] << ", "
                      << "Setting to 0xbb " << std::endl;
        } else {
            memset(addr[i], 0, SIZE);
            std::cout << "Allocating for target : " << addr[i] << ", "
    ...
    if (role == "target") { agent.getLocalMD(tgt_metadata); }
    ...
    for (int i = 0; i < NUM_TRANSFERS; i++) { free(addr[i]); }
    delete serdes;
