...
void test_side_perf(nixlAgent* A1, nixlAgent* A2, nixlBackendH* backend, nixlBackendH* backend2)
{
    ...
    for (int i = 0; i < (n_mems * descs_per_mem); i++) indices.push_back(i);
    ...
    indices.clear();
    for (int i = 0; i < (n_mems * descs_per_mem); i += 2) indices.push_back(i);
...
nixl_status_t partialMdTest(
        nixlAgent* A1, nixlAgent* A2, nixlBackendH* backend1, nixlBackendH* backend2)
{
    ...
    std::vector<int> indices;
    for (int i = 0; i < NUM_UPDATES * NUM_BUFFERS; i++) { indices.push_back(i); }
...
nixl_status_t sideXferTest(
        nixlAgent* A1, nixlAgent* A2, nixlXferReqH* src_handle, nixlBackendH* dst_backend)
{
    ...
    for (int i = (n_bufs / 2); i < n_bufs; i++) indices2.push_back(i);

    nixlXferReqH *req1, *req2, *req3;

    // write first half of src_bufs to dst_bufs
    status = A1->makeXferReq(
            NIXL_WRITE, src_side, indices1, dst_side, indices1, req1, &extra_params1);
    assert(status == NIXL_SUCCESS);
    ...
    for (int i = 0; i < (n_bufs / 2); i++) check_buf(dst_bufs[i], len);

    std::cout << "transfer 1 done\n";

    // read first half of dst_bufs back to second half of src_bufs
    status = A1->makeXferReq(
            NIXL_READ, src_side, indices2, dst_side, indices1, req2, &extra_params1);
    assert(status == NIXL_SUCCESS);
    ...
    for (int i = (n_bufs / 2); i < n_bufs; i++) check_buf(src_bufs[i], len);

    std::cout << "transfer 2 done\n";

    // write second half of src_bufs to dst_bufs
    status = A1->makeXferReq(
            NIXL_WRITE, src_side, indices2, dst_side, indices2, req3, &extra_params1);
    assert(status == NIXL_SUCCESS);
    ...
    for (int i = (n_bufs / 2); i < n_bufs; i++) check_buf(dst_bufs[i], len);
