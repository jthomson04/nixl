...
private:
    ...
    bool should_retry(int value, int max = 60) const { return !error() && value < max; }
    ...
    std::string makeKey(std::string name, int rank = -1) const
    {
        ...
        if (rank > -1) { suffix = "/" + std::to_string(rank); }

        return namespace_prefix + name + suffix;
    }

public:
    xferBenchEtcdRT(const std::string &etcd_endpoints, const int size, int* terminate = nullptr);
    ~xferBenchEtcdRT();
