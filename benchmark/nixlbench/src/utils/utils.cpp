...
DEFINE_string(backend, XFERBENCH_BACKEND_UCX,
        "Name of communication backend [UCX, UCX_MO, GDS] \
              (only used with nixl worker)");
DEFINE_string(initiator_seg_type, XFERBENCH_SEG_TYPE_DRAM,
        "Type of memory segment for initiator \
              [DRAM, VRAM]");
DEFINE_string(target_seg_type, XFERBENCH_SEG_TYPE_DRAM,
        "Type of memory segment for target \
              [DRAM, VRAM]");
DEFINE_string(scheme, XFERBENCH_SCHEME_PAIRWISE, "Scheme: pairwise, maytoone, onetomany, tp");
DEFINE_string(mode, XFERBENCH_MODE_SG,
...
DEFINE_uint64(total_buffer_size, 8LL * 1024 * (1 << 20),
        "Total buffer \
              size across device for each process (Default: 80 GiB)");
DEFINE_uint64(start_block_size, 4 * (1 << 10),
        "Max size of block \
              (Default: 4 KiB)");
DEFINE_uint64(max_block_size, 64 * (1 << 20),
        "Max size of block \
              (Default: 64 MiB)");
...
DEFINE_bool(gds_enable_direct, false,
        "Enable direct I/O for GDS operations (only used with GDS backend)");
// TODO: We should take rank wise device list as input to extend support
// <rank>:<device_list>, ...
// For example- 0:mlx5_0,mlx5_1,mlx5_2,1:mlx5_3,mlx5_4, ...
DEFINE_string(device_list, "all",
...
void xferBenchConfig::printConfig()
{
    ...
    if (runtime_type == XFERBENCH_RT_ETCD) {
        std::cout << std::left << std::setw(60) << "ETCD Endpoint " << ": " << etcd_endpoints
                  << std::endl;
    }
    ...
    if (worker_type == XFERBENCH_WORKER_NIXL) {
        ...
        if (backend == XFERBENCH_BACKEND_GDS) {
            ...
            std::cout << std::left << std::setw(60)
                      << "GDS enable direct (--gds_enable_direct=[0,1])" << ": "
                      << gds_enable_direct << std::endl;
        }
    }
    std::cout << std::left << std::setw(60)
              << "Initiator seg type (--initiator_seg_type=[DRAM,VRAM])" << ": "
    ...
    std::cout << std::left << std::setw(60) << "Scheme (--scheme=[pairwise,manytoone,onetomany,tp])"
              << ": " << scheme << std::endl;
    std::cout << std::left << std::setw(60) << "Mode (--mode=[SG,MG])" << ": " << mode << std::endl;
    std::cout << std::left << std::setw(60) << "Op type (--op_type=[READ,WRITE])" << ": " << op_type
              << std::endl;
    ...
    std::cout << std::left << std::setw(60) << "Num iter (--num_iter=N)" << ": " << num_iter
              << std::endl;
...
std::vector<std::string> xferBenchConfig::parseDeviceList()
{
    ...
    if (xferBenchConfig::scheme == XFERBENCH_SCHEME_PAIRWISE &&
        ...
        while (std::getline(ss, dev, ',')) { devices.push_back(dev); }
...
void xferBenchUtils::checkConsistency(std::vector<std::vector<xferBenchIOV>> &iov_lists)
{
    for (const auto &iov_list : iov_lists) {
        for (const auto &iov : iov_list) {
...
#if HAVE_CUDA
                addr = calloc(1, len);
                CHECK_CUDA_ERROR(cudaMemcpy(addr, (void*)iov.addr, len, cudaMemcpyDeviceToHost),
                        "cudaMemcpy failed");
#else
            ...
            if (true != rc) { std::cerr << "Consistency check failed\n" << std::flush; }
            free(addr);
        }
    }
}
...
void xferBenchUtils::printStats(
        bool is_target, size_t block_size, size_t batch_size, double total_duration)
{
    ...
    if (block_size > LARGE_BLOCK_SIZE) { num_iter /= LARGE_BLOCK_SIZE_ITER_FACTOR; }
    ...
    if (IS_PAIRWISE_AND_SG() && rt->getRank() != 0) { return; }
